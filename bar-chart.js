...
barButton.addEventListener("click", (event) => {
 ...
 if (!chartLabel) {
  ...
  dialog.innerHTML = "Please enter a game name before generating the ASCII chart.";
 ...
 if (checkedElements.length === 0) {
  // Display an error dialog to the user
  const dialog = document.createElement("dialog");
  dialog.innerHTML = "Please fill out the form before generating the ASCII chart.";
  document.body.appendChild(dialog);
  dialog.showModal();
  setTimeout(() => {
   dialog.close();
 ...
 const table = dataRows.map((row) => {
  return [row.categoryTitle, row.filledPoints + row.emptyPoints, row.categoryDefinition];
 });
